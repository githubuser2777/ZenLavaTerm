...
# Verify WiX Toolset (candle.exe and light.exe)
...
if (-not (Get-Command "light.exe" -ErrorAction SilentlyContinue)) {
    Write-Error "Error: light.exe (WiX Toolset) not found in PATH or standard installation directory."
    exit 1
}

Write-Host "==> Verified WiX Toolset: candle.exe at $((Get-Command candle.exe).Source), light.exe at $((Get-Command light.exe).Source)"
