...
## Platform Support & Semantic Levels

- **Linux**: Full native zero-dependency metric telemetry via `/proc/stat` (system-wide CPU ticks), `/proc/meminfo` (active/available memory), `/proc/diskstats` (system disk I/O sectors), and `/sys/class/power_supply` (battery capacity).
- **Windows**: Native zero-dependency Win32 telemetry via `kernel32` APIs (`GetSystemTimes` for CPU delta ticks, `GlobalMemoryStatusEx` for physical RAM utilization, `GetSystemPowerStatus` for battery and AC status, and `GetProcessIoCounters` for active process I/O activity).
- **macOS**: Native Darwin telemetry using Mach kernel subsystem APIs (`host_statistics64` with `HOST_CPU_LOAD_INFO` for CPU load ticks and `HOST_VM_INFO64` for memory pages), with neutral fallback baselines for battery and I/O.
- **I/O Semantic Scope**: `io_activity` is platform-adapted: Linux reads host-wide disk sector delta throughput, Windows reads process-level byte transfer counts via `GetProcessIoCounters`, and macOS uses a smooth baseline ($0.05$).
